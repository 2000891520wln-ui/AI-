...
const CONTEXT_IMAGE_KEY_PREFIX = "ai-journal-context-image:";
const contextImageByTab = new Map();
...
chrome.runtime.onMessage.addListener((message, sender) => {
  if (message?.type !== "ai-journal-context-image" || !sender.tab?.id || !message.srcUrl) return;
  contextImageByTab.set(sender.tab.id, { srcUrl: message.srcUrl, timestamp: Date.now() });
  void chrome.storage.session.set({
    [`${CONTEXT_IMAGE_KEY_PREFIX}${sender.tab.id}`]: {
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (!tab?.id) return;
  void handleContextMenuClick(info, tab.id);
});

async function handleContextMenuClick(info, tabId) {
  const srcUrl = info.srcUrl || (await readContextImage(tabId)) || (await resolveImageFromPage(tabId, info.linkUrl));
  if (!srcUrl) {
    await showToastInTab(tabId, "没有识别到卡片中的图片，请直接右键图片区域");
    return;
  }
  if (info.menuItemId === MENU_GENERATE_PROMPT) {
    await generatePrompt(srcUrl, tabId);
  }
  if (info.menuItemId === MENU_SAVE) {
    await saveToJournal(srcUrl, tabId);
  }
}

async function readContextImage(tabId) {
  const memoryImage = contextImageByTab.get(tabId);
  if (memoryImage?.srcUrl && Date.now() - memoryImage.timestamp <= 15_000) return memoryImage.srcUrl;
  const key = `${CONTEXT_IMAGE_KEY_PREFIX}${tabId}`;
  const stored = await chrome.storage.session.get(key);
  const contextImage = stored[key];
  if (!contextImage?.srcUrl || Date.now() - Number(contextImage.timestamp || 0) > 15_000) return "";
  return contextImage.srcUrl;
}

async function resolveImageFromPage(tabId, linkUrl = "") {
  const results = await chrome.scripting.executeScript({
    ...
    func: (contextLinkUrl) => {
      ...
      const scanRoot = (root, basePriority) => {
        if (!(root instanceof Element)) return;
        if (root instanceof HTMLImageElement) addImage(root, basePriority + 10);
        root.querySelectorAll("img").forEach((image) => addImage(image, basePriority));
        Array.from(root.querySelectorAll("[style]")).slice(0, 120).forEach((element) => {
          const backgroundImage = getComputedStyle(element).backgroundImage;
          const backgroundUrl = backgroundImage.match(/url\(["']?(.*?)["']?\)/)?.[1];
          if (backgroundUrl) candidates.push({ src: backgroundUrl, priority: basePriority - 2, area: element.clientWidth * element.clientHeight });
        });
      };

      if (contextLinkUrl) {
        const matchingLinks = Array.from(document.querySelectorAll("a[href]")).filter((anchor) => anchor.href === contextLinkUrl);
        matchingLinks.forEach((anchor) => {
          ...
          for (let depth = 0; card && depth < 4; depth += 1, card = card.parentElement) {
            ...
            scanRoot(card, 130 - depth * 10);
          }
        });
      }

      let target = hovered[hovered.length - 1];
      if (target instanceof HTMLImageElement) addImage(target, 120);
      for (let depth = 0; target && depth < 8; depth += 1, target = target.parentElement) {
        if (target === document.body || target === document.documentElement) break;
        scanRoot(target, 110 - depth * 10);
        const backgroundImage = getComputedStyle(target).backgroundImage;
        const backgroundUrl = backgroundImage.match(/url\(["']?(.*?)["']?\)/)?.[1];
        if (backgroundUrl) candidates.push({ src: backgroundUrl, priority: 105 - depth * 10, area: target.clientWidth * target.clientHeight });
      }
...
async function postApi(settings, path, payload) {
  ...
  for (const base of bases) {
    try {
      ...
      if (isTerminalAiError(error)) throw new Error(friendlyAiError(error));
...
async function analyzePrompt(settings, imageDataUrl, srcUrl) {
  try {
    ...
    if (isTerminalAiError(primaryError)) throw new Error(friendlyAiError(primaryError));
    try {
...
function isTerminalAiError(error) {
  const message = errorMessage(error).toLowerCase();
  return (
    message.includes("overdue balance") ||
    message.includes("insufficient balance") ||
    message.includes("insufficient funds") ||
    message.includes("余额不足") ||
    message.includes("账户欠费") ||
    message.includes("账号欠费") ||
    message.includes("额度不足")
  );
}

function friendlyAiError(error) {
  return isTerminalAiError(error) ? "余额不足：请更新服务端 AI API Key 或为对应账户充值后重试" : errorMessage(error);
}
